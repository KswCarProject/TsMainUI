...
public class CanCadillacAtsTpmsActivity extends CanBaseActivity implements UserCallBack, View.OnClickListener, CanItemProgressList.onPosChange {
    public static final int ITEM_VOL = 0;
    public static final String TAG = "CanCadillacAtsTpmsActivity";
    ...
    public void InitUI_1280x480() {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            this.mIvTyres[i].setStateDrawable(R.drawable.can_rf_tpms_up, R.drawable.can_rf_tpms_dn);
            this.mTvPress[i] = this.mManager.AddCusText(((i % 2) * KeyDef.SKEY_POWEWR_1) + 113, ((i / 2) * 171) + 50, 281, 50);
            this.mTvPress[i].SetPixelSize(47);
            this.mTvPress[i].setGravity(17);
        }
    }
    ...
    private void ResetData(boolean check) {
        ...
        if (!check || i2b(this.mData.Update)) {
            this.mData.Update = 0;
            SetVal(0, this.mData.Tpms[0]);
            SetVal(1, this.mData.Tpms[1]);
            SetVal(2, this.mData.Tpms[2]);
            SetVal(3, this.mData.Tpms[3]);
        }
    }
