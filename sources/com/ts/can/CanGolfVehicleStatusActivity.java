...
public class CanGolfVehicleStatusActivity extends CanGolfBaseActivity implements View.OnClickListener, CanItemMsgBox.onMsgBoxClick, UserCallBack {
    ...
    private CanDataInfo.GolfTpmsData mTpmsData = new CanDataInfo.GolfTpmsData();
    protected TextView[] mTpmsTip;
    private CanDataInfo.GolfTpmsWarn mTpmsWarn = new CanDataInfo.GolfTpmsWarn();
            ...
            if (i2b(this.mReportData.UpdateOnce)) {
                if (!check || i2b(this.mReportData.Update)) {
                    ...
                    if (this.mReportData.Num == 0) {
                        ...
                        this.mBtnReports.setText(String.format("%d %s", new Object[]{Integer.valueOf(this.mReportData.Num), getResources().getString(R.string.can_gf_reports)}));
                    }
                }
            } else if (!check) {
    ...
    private void setTyresValues(int unit, int[] realValues, int[] referenceValues) {
        for (int i = 0; i < this.mRealTyres.length; i++) {
            if (unit == 0) {
                ...
                this.mRealTyres[i].setText(String.format("%.1f", new Object[]{Float.valueOf(((float) realValues[i]) / 2.0f)}));
                this.mReferenceTyres[i].setText(String.format("(%.1f)", new Object[]{Float.valueOf(((float) referenceValues[i]) / 2.0f)}));
            } else {
                this.mRealTyres[i].setText(String.format("%.1f", new Object[]{Float.valueOf(((float) realValues[i]) * 10.0f)}));
                this.mReferenceTyres[i].setText(String.format("(%.1f)", new Object[]{Float.valueOf(((float) referenceValues[i]) * 10.0f)}));
            }
    ...
    private void ShowPage() {
        ...
        if (CanJni.GetSubType() == 3 || isRzcMix()) {
            ...
            setViewShow((View) this.mTyresWarm, bTyresShow);
        }
    ...
    public void onClick(View v) {
        ...
        switch (Id) {
            ...
            case 5:
                ...
                return;
            case 6:
                new CanPopupDialog((Context) this, this.mTyresArray, this.mTyresData.Zjstyjc, v, (CanPopupDialog.OnItemClick) new CanPopupDialog.OnItemClick() {
