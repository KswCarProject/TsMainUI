...
public class CanAudiLzWithCdACBox implements UserCallBack {
    private static final String TAG = "CanAudiLzWithCdACBox";
    ...
    private static CanAudiLzWithCdACBox sInstance;
    private CanDataInfo.CAN_ACInfo mACInfo;
    private CanDataInfo.AudiWithCdLz_AirInfo mAirInfo = new CanDataInfo.AudiWithCdLz_AirInfo();
    ...
    private CanAudiLzWithCdACBox() {
    }

    public static CanAudiLzWithCdACBox GetInstance() {
        if (sInstance == null) {
            sInstance = new CanAudiLzWithCdACBox();
        }
        return sInstance;
    }
    ...
    public void ShowAC() {
        if (!mIsAC) {
            CanJni.AudiLzGetAirInfo(this.mAirInfo);
            if (i2b(this.mAirInfo.fgLtemp) || i2b(this.mAirInfo.fgRtemp) || this.mAirInfo.fgWind != 0 || this.mAirInfo.fgMode != 0) {
                mfgJump = true;
                onResume();
            }
        }
    ...
    private void ResetData(boolean check) {
        ...
        if (this.mAirInfo.UpdateOnce != 0 && (!check || this.mAirInfo.Update != 0)) {
            ...
            boolean showLtTemp = i2b(this.mAirInfo.fgLtemp);
            boolean showRtTemp = i2b(this.mAirInfo.fgRtemp);
            boolean showLtWind = i2b(this.mAirInfo.fgWind);
            if (!isDoubleAirMode() || !i2b(this.mAirInfo.fgWind)) {
                showRtWind = false;
            } else {
                showRtWind = true;
            }
            boolean showLtMode = i2b(this.mAirInfo.fgMode);
            if (!isDoubleAirMode() || !i2b(this.mAirInfo.fgMode)) {
                showRtMode = false;
            } else {
                showRtMode = true;
            }
