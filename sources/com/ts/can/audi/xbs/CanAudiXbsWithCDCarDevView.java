...
public class CanAudiXbsWithCDCarDevView extends CanRelativeCarInfoView {
    ...
    public CanAudiXbsWithCDCarDevView(Activity activity) {
        super(activity);
    }
    ...
    public void doOnResume() {
        ...
        CanJni.AudiWithCdXbsSrcSwitch(1);
    }
    ...
    public void doOnPause() {
        ...
        CanJni.AudiWithCdXbsSrcSwitch(2);
    }

    public static void AudiWithCDDModeChange(int sta) {
        if (sta == 0) {
            int temp = 0;
            if (CanAudiXbsWithCDCarInitView.IsAuxConfig() > 0) {
                temp = 0 | 128;
            }
            CanJni.AudiWithCdXbsAudioSet(1, temp | ((CanAudiXbsWithCDCarInitView.IsAuxLin2() + 1) << 4) | ((CanAudiXbsWithCDCarInitView.IsAuxLin1() + 1) & 15));
        }
    }
    ...
    public static void Init() {
        if (CanAudiXbsWithCDCarFuncView.RvsMode() == 1) {
            Mcu.SendXKey(33);
        } else if (CanAudiXbsWithCDCarFuncView.RvsMode() == 2) {
            Mcu.SendXKey(35);
        } else if (CanAudiXbsWithCDCarFuncView.RvsMode() == 3) {
            Mcu.SendXKey(36);
        } else {
            Mcu.SendXKey(34);
        }
        if (CanFunc.getInstance().IsCore() == 1) {
            if (CanAudiXbsWithCDCarFuncView.RCamera() <= 0 || CanAudiXbsWithCDCarFuncView.RvsMode() == 2) {
                Mcu.SendXKey(40);
            } else {
                Mcu.SendXKey(41);
            }
        } else if (CanAudiXbsWithCDCarFuncView.RCamera() > 0) {
            Mcu.SendXKey(41);
        } else {
            Mcu.SendXKey(40);
        }
        Mcu.SendXKey(CanAudiXbsWithCDCarFuncView.RvsDelay() + 42);
        Mcu.SendXKey(((FtSet.Getlgb3() & 61440) >> 12) + 50);
        CanAudiXbsWithCDCarInitView.SetCamType(0, 0, 0);
        if (CanAudiXbsWithCDCarInitView.IsLvdsType() == 0 && CanFunc.getInstance().IsCore() == 1) {
            Mcu.ReqOrgTiming(32);
        } else if (CanAudiXbsWithCDCarInitView.IsLvdsType() == 1 && CanFunc.getInstance().IsCore() == 1) {
            Mcu.ReqOrgTiming(33);
        }
        Intent intent = new Intent("com.ts.can.BROADCAST_CAN_INFO");
        intent.putExtra("CanType", CanJni.GetCanType());
        intent.putExtra("CanSubType", CanAudiXbsWithCDCarInitView.DoorUI());
        MyApplication.mContext.sendBroadcast(intent);
        byte[] fileMsg = new byte[8];
        fileMsg[0] = (byte) FtSet.GetCanTp();
        fileMsg[1] = (byte) CanAudiXbsWithCDCarInitView.DoorUI();
