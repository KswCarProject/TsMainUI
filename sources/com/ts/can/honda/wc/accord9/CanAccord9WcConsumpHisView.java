...
public class CanAccord9WcConsumpHisView extends CanRelativeCarInfoView {
    ...
    public CanAccord9WcConsumpHisView(Activity activity) {
        super(activity);
    }
    ...
    public void onClick(View v) {
        switch (((Integer) v.getTag()).intValue()) {
            case 1:
                enterSubWin(CanCarInfoSub1Activity.class, 0);
                return;
            case 2:
                CanJni.HondaWcAccord9PcSet(1, 1, 1, 255);
                CanJni.HondaWcAccord9PcSet(2, 1, 1, 255);
                return;
            default:
                return;
        }
    ...
    public void ResetData(boolean check) {
        ...
        if (!check || i2b(this.mHistoryData.Update)) {
            ...
            int Yhlc = getCurDataYhlc(this.mHistoryData.Yhlc);
            ...
            for (int i = 1; i < 4; i++) {
                this.mTripA[i].setText(GetTripAVal(this.mTripAData[i], this.mHistoryData.TripaDw));
                Log.d("lq", "mHistoryData.PjyhDw:" + this.mHistoryData.PjyhDw);
                this.mConsump[i].setText(GetYhVal(this.mConsumpData[i], this.mHistoryData.PjyhDw));
                this.mProg[i].SetMinMax(0, maxpos);
                if (65535 == this.mConsumpData[i]) {
                    this.mProg[i].SetCurPos(0);
                } else if (this.mConsumpData[i] >= maxpos) {
                    this.mProg[i].SetCurPos(maxpos);
                } else {
