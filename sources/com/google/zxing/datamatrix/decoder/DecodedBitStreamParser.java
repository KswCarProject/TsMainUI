...
final class DecodedBitStreamParser {
        ...
        while (bits.available() != 8 && (firstByte = bits.readBits(8)) != 254) {
            ...
            for (int i = 0; i < 3; i++) {
                ...
                switch (shift) {
                    ...
                    case 3:
                        if (upperShift) {
                            result.append((char) (cValue + Can.CAN_ZH_H530));
                            upperShift = false;
                        } else {
    ...
    private static void parseTwoBytes(int firstByte, int secondByte, int[] result) {
        ...
        int temp = fullBitValue / CanCameraUI.BTN_FENGSHEN_AX7_MODE_UP;
        result[0] = temp;
        int fullBitValue2 = fullBitValue - (temp * CanCameraUI.BTN_FENGSHEN_AX7_MODE_UP);
    ...
    private static void decodeBase256Segment(BitSource bits, StringBuilder result, Collection<byte[]> byteSegments) throws FormatException {
        ...
        if (d1 == 0) {
            ...
            count = ((d1 - 249) * Can.CAN_NISSAN_XFY) + unrandomize255State(bits.readBits(8), codewordPosition3);
        }
        if (count < 0) {
            throw FormatException.getFormatInstance();
        }
